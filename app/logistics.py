...
def cluster_collection_routes(db: Session, n_clusters: int = 3) -> list[dict]:
    ...
    Returns a list of routes with stops. Falls back to flat list if scikit-learn unavailable.
    """
    ...
    if not _HAS_SKLEARN:
        return [
            {
                "id": f"RT-{secrets.token_hex(2).upper()}-0",
        ...
        routes.append(
            {
                ...
                "center": {
                    "lat": float(centroids[cid][0]),
                    "lng": float(centroids[cid][1]),
                },
                "total_stops": len(stops),
                "stops": stops,
                "estimated_kg": sum([s.get("kg", 0) for s in stops]),
            }
        )
