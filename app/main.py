...
def _check_chat_throttle(sender_key: str):
    now = time.time()
    if now - _chat_last_send.get(sender_key, 0.0) < 2.0:
...
async def fw_webhook(request: Request, db: Session = Depends(get_session)):
    ...
    if FW_SECRET_KEY and sig != FW_SECRET_KEY:
        return JSONResponse({"error": "unauthorized"}, status_code=401)
    data = await request.json()
    if data.get("status") == "successful":
        tx_id, tx_ref = (
            data.get("data", {}).get("id"),
            data.get("data", {}).get("tx_ref"),
        )
        v = await verify_fw_transaction(tx_id)
        if v.get("status") == "success":
            payments.confirm(db, tx_ref, True)
...
def login(
    ...
    if (
        req.role == "agent"
        and hmac.compare_digest(req.pin_or_password, _AGENT_PASSWORD)
        and req.id_or_phone == "admin"
    ):
        return {
            "ok": True,
    ...
    if (
        req.role == "supervisor"
        and hmac.compare_digest(req.pin_or_password, _SUPERVISOR_PASSWORD)
        and req.id_or_phone == "admin"
    ):
        return {
            "ok": True,
    ...
    stmt = select(User).where(
        (User.phone == req.id_or_phone) | (User.id == req.id_or_phone.upper())
    )
    user = db.execute(stmt).scalar_one_or_none()
    if (
        user
        and user.role == req.role
        and hmac.compare_digest(req.pin_or_password, user.password_hash)
    ):
        token = issue_session(response, user.role, user.id)
        red = (
            f"/farmer/{user.id}"
            if user.role == "farmer"
            else (f"/buyer/{user.id}" if user.role == "buyer" else "/logistics")
        )
...
def farmer_dash(
    farm_id: str,
    user: dict = Depends(require_user("farmer", "agent")),
    db: Session = Depends(get_session),
):
...
def payments_for_buyer(
    buyer_id: str,
    db: Session = Depends(get_session),
    user: dict = Depends(require_user("buyer", "agent")),
):
    require_owner_or_agent("buyer", buyer_id, user)
...
def crp_offers_list(
    ...
    if user["role"] == "farmer" and (not farm_id or farm_id != user["subject"]):
        raise HTTPException(status_code=403)
    return crp.list_open_offers(
        db, limit=limit, farm_id=farm_id, include_closed=include_closed
    )


@app.post("/sensor/telemetry")
async def sensor_telemetry(
    req: TelemetryRecord,
    db: Session = Depends(get_session),
    user: dict = Depends(require_user()),
):
...
def list_training_modules(
    db: Session = Depends(get_session), user: dict = Depends(require_user())
):
...
def crp_ask(
    ...
    farm_id = (
        req.farm_id
        if user["role"] in ["agent", "buyer", "supervisor"]
        else user["subject"]
    )
...
def onboarding_status(
    user: dict = Depends(require_user("farmer")), db: Session = Depends(get_session)
):
    f = db.execute(
        select(FarmerProfile).where(FarmerProfile.user_id == user["subject"])
    ).scalar_one_or_none()
    if not f:
...
async def onboarding_kyc(
    ...
    photo_url = await blobs.upload_file(file, "kyc", user["subject"])

    f.verification_status = "pending_device"
    db.commit()
    ledger.write(
        "KYC_SUBMITTED",
        {"farm_id": user["subject"], "nin": document_id, "id_photo": photo_url},
    )
...
def onboarding_purchase(
    ...
    send_sms(
        "+256770000000",
        f"Alert: Farm {f.farmer_name} ({f.district}) needs hardware installation.",
    )
...
def list_farms(db: Session = Depends(get_session)):
    ...
    return {
        f.user_id: {
            ...
            "district": f.district,
            "crop": f.crop,
            "yps": scorer.score_farm(db, f.user_id).get("yps", 0)
...
def get_finance_status(farm_id: str, db: Session = Depends(get_session)):
    ...
    settled = (
        db.execute(
            select(Settlement).where(
                Settlement.farm_id == farm_id, Settlement.status == "settled"
            )
        )
        .scalars()
        .all()
    ...
    pending = (
        db.execute(
            select(Settlement).where(
                Settlement.farm_id == farm_id, Settlement.status == "pending"
            )
        )
        .scalars()
        .all()
    ...
    return {
        ...
        "recent_payments": [
            dict(id=s.id, amount=s.amount_ugx, status=s.status, date=s.created_at)
            for s in settled[-5:]
        ],
    }
...
def supervisor_stats(
    user: dict = Depends(require_user("supervisor")), db: Session = Depends(get_session)
):
    # Regional YPS breakdown
    regional_yps = []
    districts = (
        db.execute(text("SELECT DISTINCT district FROM farmer_profiles"))
        .scalars()
        .all()
    )
    for d in districts:
        scores = [
            scorer.score_farm(db, f.user_id).get("yps", 0)
            for f in db.execute(
                select(FarmerProfile).where(FarmerProfile.district == d)
            )
            .scalars()
            .all()
        ]
        if scores:
            regional_yps.append(
                {"district": d, "avg_yps": int(sum(scores) / len(scores))}
            )
...
def signup(req: SignupReq, response: Response, db: Session = Depends(get_session)):
    user_id = ("UG-FARM-" if req.role == "farmer" else "BY-") + secrets.token_hex(
        3
    ).upper()
    user = User(
        id=user_id, phone=req.phone, role=req.role, password_hash=req.pin_or_password
    )
    db.add(user)
    db.flush()
...
def agent_overview(
    db: Session = Depends(get_session), user: dict = Depends(require_user("agent"))
):
    ...
    for f in farmers:
        ...
        # Get active priority
        priority = db.execute(
            select(YieldPriority).where(
                YieldPriority.farm_id == f.user_id, YieldPriority.status == "active"
            )
        ).scalar_one_or_none()

        if priority:
            total_kg_allocated += priority.kg_allocated
            ...
            triage_alerts.append(
                {
                    ...
                    "name": f.farmer_name,
                    "issue": "Low Moisture"
                    if latest_t[0].soil_moisture < 20
                    else "Low YPS",
                }
            )
    ...
    flagged = (
        db.execute(select(Post).where(Post.hidden == True).limit(10)).scalars().all()
    )
    ...
    unread_n = (
        db.execute(
            select(Notification).where(
                Notification.user_id == user["subject"], Notification.read == False
            )
        )
        .scalars()
        .all()
...
def api_agent_verify(
    farm_id: str,
    db: Session = Depends(get_session),
    user: dict = Depends(require_user("agent")),
):
    f = db.execute(
        select(FarmerProfile).where(FarmerProfile.user_id == farm_id)
    ).scalar_one_or_none()
...
def api_agent_moderate(
    ...
    ledger.write(
        "POST_MODERATED",
        {"post_id": post_id, "action": action, "agent": user["subject"]},
    )
    return {"ok": True}


@app.post("/api/agent/priorities/issue")
def api_agent_priority_issue(
    req: PriorityApproveRequest,
    db: Session = Depends(get_session),
    user: dict = Depends(require_user("agent")),
):
    ...
    existing = db.execute(
        select(YieldPriority).where(
            YieldPriority.farm_id == req.farm_id, YieldPriority.status == "active"
        )
    ).first()
    if existing:
        return JSONResponse({"error": "already_has_active_priority"}, status_code=400)

    score = scorer.score_farm(db, req.farm_id)
    if "error" in score:
        return JSONResponse(score, status_code=400)
    ...
    new_p = YieldPriority(
        ...
        farm_id=req.farm_id,
        yps=score["yps"],
        kg_allocated=score["kg_allocated"],
        kg_remaining=score["kg_allocated"],
        expires_at=expires,
        signature="SIG-" + secrets.token_hex(16),
    )
    db.add(new_p)
    db.commit()
    ledger.write(
        "PRIORITY_ISSUED",
        {"priority_id": pid, "farm_id": req.farm_id, "yps": score["yps"]},
    )
...
@app.get("/api/feed")
def get_feed(
    limit: int = 50, district: Optional[str] = None, db: Session = Depends(get_session)
):
...
def get_verified_feed(limit: int = 50, db: Session = Depends(get_session)):
    ...
    stmt = (
        select(Post)
        .where(Post.is_verified == True, Post.hidden == False)
...
def create_feed_post(
    ...
    return social.create_post(
        db, user["subject"], req.body, req.photo_url, req.is_verified, req.metadata
    )
...
def social_bid(
    ...
    ledger.write(
        "SOCIAL_BID_INITIATED",
        {"post_id": post_id, "buyer_id": user["subject"], "thread_id": thread["id"]},
    )
...
async def upload_harvest_photo(
    file: UploadFile = File(...), user: dict = Depends(require_user("farmer"))
):
    """Handles low-bandwidth WebP image uploads via Vercel Blob (or local fallback)."""
    photo_url = await blobs.upload_file(file, "harvest", user["subject"])
...
def logistics_pending(
    db: Session = Depends(get_session),
    user: dict = Depends(require_user("logistics", "agent")),
):
...
def logistics_advise_ai(
    req: LogisticsAdviseRequest,
    user: dict = Depends(require_user("logistics", "agent")),
):
...
@app.get("/ledger")
def ledger_view(
    user: dict = Depends(require_user("agent")), db: Session = Depends(get_session)
):
...
def hardware_heartbeat(req: HeartbeatReq, db: Session = Depends(get_session)):
    ...
    return hardware.log_heartbeat(
        db, req.farm_id, req.device_id, req.firmware, req.battery, req.rssi
    )


@app.get("/api/agent/hardware/status")
def agent_hardware_status(
    db: Session = Depends(get_session), user: dict = Depends(require_user("agent"))
):
