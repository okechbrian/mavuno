...
def run_alembic_migrations():
    """Runs Alembic migrations programmatically. Falls back to create_all."""
    try:
        ...
        print(f"Database migrated ({engine.name}).")
        return True
...
def init_db():
    """Initializes the database using Alembic migrations or create_all fallback."""
    if not run_alembic_migrations():
        Base.metadata.create_all(bind=engine)
        print(f"Database tables created ({engine.name}).")
        ...
        if path.exists():
            with open(path) as f:
                ...
                for fid, info in data.items():
                    user = User(
                        id=fid, phone=info["phone"], role="farmer", password_hash="1234"
                    )
                    db.add(user)
                    profile = FarmerProfile(
                        ...
                        drought_factor=info.get("drought_factor", 1.0),
                    )
                    db.add(profile)
            ...
            with open(path) as f:
                ...
                for bid, info in data.items():
                    ...
                    profile = BuyerProfile(
                        ...
                        name=info["name"],
                        region=info["region"],
                        crops_json=info["crops_json"],
                        floor_ugx=info["floor_ugx"],
                        radius_km=info.get("radius_km", 50.0),
                        lat=info["lat"],
                        lng=info["lng"],
                    )
                    db.add(profile)
